...
async function seed() {
  ...
  // THEMATICS
  const [themFr, themJp, themUs, themPatisserie, themStreet, themMedieval] = await Promise.all([
    prisma.thematic.create({ data: { name: 'Cuisine française', description: 'Recettes inspirées de la gastronomie française traditionnelle.' } }),
    prisma.thematic.create({ data: { name: 'Cuisine japonaise', description: 'Recettes issues de la tradition culinaire japonaise.' } }),
    prisma.thematic.create({ data: { name: 'Cuisine américaine', description: 'Plats typiques de la culture food américaine.' } }),
    prisma.thematic.create({ data: { name: 'Pâtisserie & Desserts', description: 'Gâteaux, entremets et douceurs sucrées.' } }),
    prisma.thematic.create({ data: { name: 'Street Food & Snacks', description: 'Recettes rapides, sandwichs et plats du quotidien.' } }),
    prisma.thematic.create({ data: { name: 'Cuisine médiévale', description: "Recettes s'inspirant des festins et banquets de l'ère médiévale." } }),
  ]);
  ...
  const recRatatouille = await prisma.recipe.create({
    data: {
      ...
      description: "La célèbre ratatouille confit qui a fait chavirer le cœur du critique Anton Ego. Fines tranches de légumes en rosace sur un coulis de tomate parfumé au thym.",
      image: 'https://placehold.co/800x600?text=Ratatouille+Confit',
      prepTime: 45, cookTime: 90, servings: 4,
      difficulty: 'HARD', state: 'APPROVED',
      userId: admin.id, workId: workRatatouille.id,
    },
  });
  ...
  await Promise.all([
    prisma.step.createMany({
      data: [
        { order: 1, content: "Coulis : mixer 4 tomates pelées avec 2 gousses d'ail et l'oignon. Réduire 20 min à feu doux avec un filet d'huile d'olive. Saler, poivrer.", recipeId: recRatatouille.id },
        { order: 2, content: 'Trancher à la mandoline les courgettes, aubergines, tomates et poivrons en rondelles de 2-3 mm. La régularité est essentielle pour une cuisson homogène.', recipeId: recRatatouille.id },
        { order: 3, content: 'Napper le fond du plat avec le coulis. Disposer les légumes en rosace alternée en les faisant se chevaucher légèrement.', recipeId: recRatatouille.id },
        { order: 4, content: "Assaisonner avec sel, poivre, thym et un filet d'huile d'olive. Couvrir d'un papier sulfurisé découpé aux dimensions du plat.", recipeId: recRatatouille.id },
        { order: 5, content: "Cuire à 135°C pendant 1h30. Les légumes doivent être fondants et légèrement caramélisés. Servir en rosace à l'assiette.", recipeId: recRatatouille.id },
      ],
    }),
    ...
    prisma.recipeThematic.create({ data: { recipeId: recRatatouille.id, thematicId: themFr.id } }),
  ]);
  ...
  const workJulieJulia = await prisma.work.create({
    data: {
      title: 'Julie & Julia',
      releaseYear: 2009,
      synopsis: "Julie Powell cuisine les 524 recettes de Julia Child en 365 jours, en parallèle de la vie de la cuisinière en France dans les années 50.",
  ...
  const recBourguignon = await prisma.recipe.create({
    data: {
      ...
      description: "Le plat emblématique du livre « Mastering the Art of French Cooking ». Braisé de bœuf au Bourgogne, lardons et champignons, mijoté jusqu'à la perfection absolue.",
      image: 'https://placehold.co/800x600?text=Boeuf+Bourguignon',
      prepTime: 40, cookTime: 180, servings: 6,
      difficulty: 'MEDIUM', state: 'APPROVED',
      userId: julia.id, workId: workJulieJulia.id,
    },
  });
  ...
  await Promise.all([
    prisma.step.createMany({
      data: [
        { order: 1, content: "Sécher la viande au papier absorbant. Faire revenir les lardons dans le beurre en cocotte, réserver. Faire dorer le bœuf en plusieurs fois à feu vif.", recipeId: recBourguignon.id },
        { order: 2, content: "Dans la cocotte, faire suer l'oignon et les carottes. Saupoudrer de farine et cuire 2 min (singer). Remettre viande et lardons.", recipeId: recBourguignon.id },
        { order: 3, content: "Mouiller avec le vin rouge jusqu'à hauteur. Ajouter le laurier et le thym. Porter à ébullition, couvrir, enfourner à 160°C pendant 2h30.", recipeId: recBourguignon.id },
        { order: 4, content: "Faire sauter les champignons dans le beurre à feu vif. Les incorporer 30 min avant la fin. Rectifier l'assaisonnement. Servir avec des pommes de terre vapeur.", recipeId: recBourguignon.id },
      ],
    }),
    prisma.recipeIngredient.createMany({
      data: [
    ...
    prisma.recipeThematic.create({ data: { recipeId: recBourguignon.id, thematicId: themFr.id } }),
  ]);
  ...
  const workChef = await prisma.work.create({
    data: {
      title: 'Chef',
      releaseYear: 2014,
      synopsis: "Un chef renommé quitte son restaurant pour ouvrir un food truck et retrouver sa créativité en parcourant les États-Unis avec son fils.",
  ...
  const recCubano = await prisma.recipe.create({
    data: {
      ...
      description: "Le sandwich iconique du food truck. Porc effiloché, jambon, emmental fondant, cornichons croquants et moutarde entre deux tranches de pain pressées à la perfection.",
      image: 'https://placehold.co/800x600?text=Cubano',
      prepTime: 15, cookTime: 10, servings: 2,
      difficulty: 'EASY', state: 'APPROVED',
      userId: gusteau.id, workId: workChef.id,
    },
  });
  ...
  await Promise.all([
    prisma.step.createMany({
      data: [
        { order: 1, content: "Tartiner généreusement les faces intérieures du pain avec la moutarde de Dijon.", recipeId: recCubano.id },
        { order: 2, content: "Garnir avec les tranches de jambon, le porc effiloché réchauffé, les rondelles de cornichons et les tranches d'emmental.", recipeId: recCubano.id },
        { order: 3, content: "Faire fondre le beurre dans une poêle à feu moyen. Déposer le sandwich et appuyer avec une spatule ou un poids. Cuire 3-4 min par face jusqu'à pain doré et fromage fondu.", recipeId: recCubano.id },
        { order: 4, content: "Couper en diagonale. Servir immédiatement — le fromage doit filer et le pain craquer sous la dent.", recipeId: recCubano.id },
      ],
    }),
    prisma.recipeIngredient.createMany({
      data: [
    ...
    prisma.recipeThematic.createMany({
      data: [
        { recipeId: recCubano.id, thematicId: themUs.id },
        { recipeId: recCubano.id, thematicId: themStreet.id },
      ],
    }),
  ]);
  ...
  const workChocolat = await prisma.work.create({
    data: {
      title: 'Chocolat',
      releaseYear: 2000,
      synopsis: "Vianne Thierry ouvre une chocolaterie dans un village conservateur français, bousculant les habitants en plein Carême.",
  ...
  const recChocolat = await prisma.recipe.create({
    data: {
      ...
      description: "La mousse au chocolat diaboliquement fondante de Vianne Thierry, avec une touche de piment chipotle pour réchauffer les âmes les plus froides du village.",
      image: 'https://placehold.co/800x600?text=Mousse+Chocolat',
      prepTime: 20, cookTime: 5, servings: 6,
      difficulty: 'EASY', state: 'PENDING',
      userId: admin.id, workId: workChocolat.id,
    },
  });
  ...
  await Promise.all([
    prisma.step.createMany({
      data: [
        { order: 1, content: "Faire fondre le chocolat noir au bain-marie avec la crème liquide chaude. Ajouter une pincée de chipotle. Laisser tiédir à 35°C.", recipeId: recChocolat.id },
        { order: 2, content: "Incorporer les jaunes d'œufs un par un au chocolat fondu en fouettant énergiquement.", recipeId: recChocolat.id },
        { order: 3, content: "Monter les blancs en neige ferme avec le sucre. Les incorporer délicatement en trois fois par mouvements enveloppants pour ne pas les casser.", recipeId: recChocolat.id },
        { order: 4, content: "Répartir dans des verrines et réfrigérer au minimum 3 heures. Décorer d'un copeau de chocolat noir avant de servir.", recipeId: recChocolat.id },
      ],
    }),
    prisma.recipeIngredient.createMany({
      data: [
    ...
    prisma.recipeThematic.createMany({
      data: [
        { recipeId: recChocolat.id, thematicId: themPatisserie.id },
        { recipeId: recChocolat.id, thematicId: themFr.id },
      ],
    }),
  ]);
  ...
  const workGOT = await prisma.work.create({
    data: {
      ...
      releaseYear: 2011,
      synopsis: "Dans les royaumes de Westeros, de grandes familles se disputent le Trône de Fer dans une guerre épique mêlant politique, trahison et magie.",
  ...
  const recTourte = await prisma.recipe.create({
    data: {
      ...
      description: "Inspirée des fastueux banquets de Port-Réal. Pâte dorée et croustillante, garnie de champignons sauvages, fromage de chèvre fondant et crème fraîche.",
      image: 'https://placehold.co/800x600?text=Tourte+Medievale',
      prepTime: 30, cookTime: 45, servings: 6,
      difficulty: 'MEDIUM', state: 'APPROVED',
      userId: julia.id, workId: workGOT.id,
    },
  });
  ...
  await Promise.all([
    prisma.step.createMany({
      data: [
        { order: 1, content: "Faire revenir les champignons tranchés dans le beurre avec l'ail et le thym jusqu'à évaporation totale de l'eau de végétation. Saler, poivrer. Réserver.", recipeId: recTourte.id },
        { order: 2, content: "Mélanger les champignons refroidis avec la crème fraîche, le chèvre émietté et 2 œufs battus. Rectifier l'assaisonnement.", recipeId: recTourte.id },
        { order: 3, content: "Foncer un moule à tarte avec les 2/3 de la pâte brisée. Piquer le fond. Verser la garniture.", recipeId: recTourte.id },
        { order: 4, content: "Couvrir avec le reste de pâte, souder les bords, dorer à l'œuf battu. Pratiquer une cheminée au centre. Cuire à 180°C pendant 40-45 min.", recipeId: recTourte.id },
      ],
    }),
    prisma.recipeIngredient.createMany({
      data: [
    ...
    prisma.recipeThematic.create({ data: { recipeId: recTourte.id, thematicId: themMedieval.id } }),
  ]);
  ...
  const workFriends = await prisma.work.create({
    data: {
      title: 'Friends',
      releaseYear: 1994,
      synopsis: "Six amis new-yorkais naviguent ensemble à travers les hauts et les bas de la vie adulte, de l'amour et du travail à Manhattan.",
  ...
  const recMoistMaker = await prisma.recipe.create({
    data: {
      ...
      description: "Le sandwich de Thanksgiving le plus célèbre de la télévision. La tranche centrale imbibée de jus de dinde — le fameux Moist Maker — fait toute la différence. On ne rigole pas avec le sandwich de Ross.",
      image: 'https://placehold.co/800x600?text=Moist+Maker',
      prepTime: 10, cookTime: 0, servings: 1,
      difficulty: 'EASY', state: 'PENDING',
      userId: gusteau.id, workId: workFriends.id,
    },
  });
  ...
  await Promise.all([
    prisma.step.createMany({
      data: [
        { order: 1, content: "Tremper généreusement une tranche de pain de mie dans le jus de cuisson de la dinde encore chaud. C'est le Moist Maker. Réserver.", recipeId: recMoistMaker.id },
        { order: 2, content: "Tartiner les deux tranches extérieures avec la sauce cranberry des deux côtés.", recipeId: recMoistMaker.id },
        { order: 3, content: "Garnir la première tranche avec de généreuses tranches de dinde rôtie et la farce aux herbes.", recipeId: recMoistMaker.id },
        { order: 4, content: "Poser le Moist Maker au centre, compléter avec une autre couche de dinde, refermer. Couper proprement. Ne JAMAIS laisser quelqu'un d'autre manger ce sandwich.", recipeId: recMoistMaker.id },
      ],
    }),
    prisma.recipeIngredient.createMany({
      data: [
    ...
    prisma.recipeThematic.createMany({
      data: [
        { recipeId: recMoistMaker.id, thematicId: themUs.id },
        { recipeId: recMoistMaker.id, thematicId: themStreet.id },
      ],
    }),
  ]);
  ...
  const workFoodWars = await prisma.work.create({
    data: {
      ...
      releaseYear: 2015,
      synopsis: "Soma Yukihira intègre l'élite de l'académie culinaire Totsuki pour devenir le meilleur chef du monde lors de duels gastronomiques.",
  ...
  const recGyudon = await prisma.recipe.create({
    data: {
      ...
      description: "Le bol de bœuf qui a vaincu les élites de Totsuki. Fines tranches de bœuf et oignons dans un tsuyu sucré-salé sur riz vapeur, couronné d'un œuf onsen fondant.",
      image: 'https://placehold.co/800x600?text=Gyudon',
      prepTime: 10, cookTime: 20, servings: 2,
      difficulty: 'EASY', state: 'APPROVED',
      userId: soma.id, workId: workFoodWars.id,
    },
  });
  ...
  await Promise.all([
    prisma.step.createMany({
      data: [
        { order: 1, content: "Cuire le riz japonais : rincer 3 fois, 1 volume riz pour 1,2 d'eau froide, porter à ébullition, couvrir et cuire 12 min à feu doux, reposer 10 min hors feu.", recipeId: recGyudon.id },
        { order: 2, content: "Dans une casserole, mélanger dashi, sauce soja, mirin et saké. Porter à frémissement. Ajouter l'oignon émincé et cuire 5 min.", recipeId: recGyudon.id },
        { order: 3, content: "Ajouter les tranches de bœuf très fines dans le bouillon frémissant. Cuire 2-3 min. Le bœuf s'imprègne de la sauce.", recipeId: recGyudon.id },
        { order: 4, content: "Servir le riz dans un grand bol. Napper du bœuf et de sa sauce. Poser délicatement un œuf onsen (63°C / 1h) sur le dessus. Parsemer de ciboulette ciselée.", recipeId: recGyudon.id },
      ],
    }),
    prisma.recipeIngredient.createMany({
      data: [
    ...
    prisma.recipeThematic.create({ data: { recipeId: recGyudon.id, thematicId: themJp.id } }),
  ]);
  ...
  const workNaruto = await prisma.work.create({
    data: {
      title: 'Naruto',
      releaseYear: 2002,
      synopsis: "Naruto Uzumaki, jeune ninja portant un démon renard en lui, rêve de devenir Hokage, le chef suprême de son village.",
  ...
  const recRamen = await prisma.recipe.create({
    data: {
      ...
      description: "Le ramen préféré de Naruto, servi chez Ichiraku Ramen à Konoha. Bouillon de porc intense et fumé, chashu fondant, œuf mariné, nori et maïs.",
      image: 'https://placehold.co/800x600?text=Ramen+Naruto',
      prepTime: 30, cookTime: 240, servings: 2,
      difficulty: 'HARD', state: 'PENDING',
      userId: soma.id, workId: workNaruto.id,
    },
  });
  ...
  await Promise.all([
    prisma.step.createMany({
      data: [
        { order: 1, content: "Bouillon : blanchir la poitrine de porc 5 min, jeter l'eau. Recommencer avec 1,5 L d'eau fraîche, oignon et ail. Mijoter 3h à frémissement. Assaisonner avec sauce soja et sel.", recipeId: recRamen.id },
        { order: 2, content: "Chashu : ficeler la poitrine. Faire revenir dans sauce soja, mirin, saké. Couvrir et mijoter 1h30 en retournant toutes les 30 min. Refroidir et trancher finement.", recipeId: recRamen.id },
        { order: 3, content: "Oeufs marinés : cuire 6 min 30 à l'eau bouillante, choc thermique, écaler. Mariner 12h dans sauce soja + mirin + eau (1:1:1).", recipeId: recRamen.id },
        { order: 4, content: "Cuire les nouilles ramen selon le paquet. Les disposer dans un grand bol chaud.", recipeId: recRamen.id },
        { order: 5, content: "Verser le bouillon brûlant. Garnir avec le chashu tranché, l'œuf coupé en deux, les feuilles de nori, le maïs et la ciboulette ciselée.", recipeId: recRamen.id },
      ],
    }),
    prisma.recipeIngredient.createMany({
      data: [
    ...
    prisma.recipeThematic.createMany({
      data: [
        { recipeId: recRamen.id, thematicId: themJp.id },
        { recipeId: recRamen.id, thematicId: themStreet.id },
      ],
    }),
  ]);
  ...
  const workWinnie = await prisma.work.create({
    data: {
      ...
      releaseYear: 1966,
      synopsis: "Dans la Forêt des Rêves Bleus, Winnie l'Ourson et ses amis vivent de douces aventures. Winnie, lui, ne vit que pour le miel.",
  ...
  const recWinnie = await prisma.recipe.create({
    data: {
      ...
      description: "Doux comme un câlin de nounours. Un cake moelleux au beurre noisette et au miel doré, parsemé d'amandes effilées, aussi réconfortant que les aventures dans les Cent Acres.",
      image: 'https://placehold.co/800x600?text=Cake+Miel',
      prepTime: 15, cookTime: 45, servings: 8,
      difficulty: 'EASY', state: 'APPROVED',
      userId: admin.id, workId: workWinnie.id,
    },
  });
  ...
  await Promise.all([
    prisma.step.createMany({
      data: [
        { order: 1, content: "Préchauffer le four à 170°C. Faire fondre le beurre jusqu'à obtenir un beurre noisette (légèrement doré et parfumé de noisette). Laisser tiédir.", recipeId: recWinnie.id },
        { order: 2, content: "Fouetter les œufs avec le sucre jusqu'à blanchiment. Ajouter le miel et le beurre noisette. Bien mélanger.", recipeId: recWinnie.id },
        { order: 3, content: "Incorporer la farine tamisée et mélanger jusqu'à homogénéité. Verser dans un moule à cake beurré et fariné.", recipeId: recWinnie.id },
        { order: 4, content: "Parsemer généreusement d'amandes effilées. Cuire 40-45 min. Laisser refroidir avant de démouler. Napper d'un filet de miel chaud au moment de servir.", recipeId: recWinnie.id },
      ],
    }),
    prisma.recipeIngredient.createMany({
      data: [
    ...
    prisma.recipeThematic.create({ data: { recipeId: recWinnie.id, thematicId: themPatisserie.id } }),
  ]);
